...
exports.SemanticFieldDetector = void 0;
class SemanticFieldDetector {
    ...
    analyzeField(fieldName, value, fullPath) {
        const matches = [];
    ...
    testPatterns(fieldName, value, patterns, category, valueType) {
        const matches = [];
    ...
    generateFhirPath(category, pattern) {
        switch (category) {
            case 'identifier':
                return 'identifier[0].value';
            case 'name':
                if (pattern.component === 'given')
                    return 'name[0].given[0]';
                if (pattern.component === 'family')
                    return 'name[0].family';
                if (pattern.component === 'prefix')
                    return 'name[0].prefix[0]';
                if (pattern.component === 'suffix')
                    return 'name[0].suffix[0]';
                return 'name[0].text';
            case 'gender':
                return 'gender';
            case 'age':
            ...
            case 'address':
                if (pattern.component === 'line')
                    return 'address[0].line[0]';
                if (pattern.component === 'city')
                    return 'address[0].city';
                if (pattern.component === 'state')
                    return 'address[0].state';
                if (pattern.component === 'postalCode')
                    return 'address[0].postalCode';
                if (pattern.component === 'country')
                    return 'address[0].country';
    ...
    getTransformer(category, pattern) {
        if (category === 'gender')
            return 'normalizeGender';
        if (category === 'age' && pattern.type !== 'birthDate')
            return 'ageToEstimatedBirthDate';
        return undefined;
    }
    getValueMap(category, pattern) {
        if (category === 'gender') {
            return {
    ...
    generateMappingConfig(detected) {
        const config = {
            identifier: [],
            name: { components: {} },
            demographics: {},
...
exports.SemanticFieldDetector = SemanticFieldDetector;

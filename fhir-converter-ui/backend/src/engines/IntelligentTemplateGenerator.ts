import { SemanticFieldDetector, DetectedField } from './SemanticFieldDetector';
import { ValuesetService } from '../services/ValuesetService';

export interface TemplateContext {
  identifiers: Array<{ path: string; system: string; value: any }>;
  name: { text?: string; given?: string; family?: string; prefix?: string; suffix?: string };
  demographics: { gender?: { path: string; valueMap?: any }; birthDate?: string; age?: string };
  telecom: Array<{ path: string; system: string; use: string }>;
  address: Array<{ path: string; component: string; use: string }>;
  extensions: Array<{ path: string; url: string; valueType: string }>;
  clinical: Array<{ path: string; code?: string; system?: string; category: string }>;
  emergency: Array<{ path: string; category: string }>;
}

export class IntelligentTemplateGenerator {
  private detector: SemanticFieldDetector;
  private valuesetService: ValuesetService;

  constructor() {
    this.detector = new SemanticFieldDetector();
    this.valuesetService = ValuesetService.getInstance();
  }

  public generateTemplate(sampleJson: any): string {
  ...
  private isFhirStructured(json: any): boolean {
    ...
    const hasFhirArrays = fhirLikeFields.some(field => 
  ...
  private generateFhirPassthroughTemplate(json: any): string {
    ...
    "name": [
      ...
      if (firstName.given && Array.isArray(firstName.given)) {
        template += `
            "given": [`;
        firstName.given.forEach((given: any, index: number) => {
          template += `
    ...
    if (json.identifier && Array.isArray(json.identifier) && json.identifier.length > 0) {
      template += `
    "identifier": [`;
      json.identifier.forEach((id: any, index: number) => {
        template += `
        {`;
        if (id.use) template += `
            "use": "{{ msg.identifier[${index}].use }}",`;
        if (id.system) template += `
            "system": "{{ msg.identifier[${index}].system }}",`;
        if (id.value) template += `
            "value": "{{ msg.identifier[${index}].value }}"`;
        template += `
        }`;
        if (index < json.identifier.length - 1) template += `,`;
      });
      template += `
    ],`;
    }
    ...
    if (json.telecom && Array.isArray(json.telecom) && json.telecom.length > 0) {
      template += `
    "telecom": [`;
      json.telecom.forEach((telecom: any, index: number) => {
        template += `
        {`;
        if (telecom.system) template += `
            "system": "{{ msg.telecom[${index}].system }}",`;
        if (telecom.value) template += `
            "value": "{{ msg.telecom[${index}].value }}",`;
        if (telecom.use) template += `
            "use": "{{ msg.telecom[${index}].use }}"`;
        template += `
        }`;
        if (index < json.telecom.length - 1) template += `,`;
      });
      template += `
    ],`;
    }
    ...
    if (json.address && Array.isArray(json.address) && json.address.length > 0) {
      template += `
    "address": [`;
      json.address.forEach((addr: any, index: number) => {
        template += `
        {`;
        if (addr.use) template += `
            "use": "{{ msg.address[${index}].use }}",`;
        if (addr.line && Array.isArray(addr.line)) {
          template += `
            "line": [`;
          addr.line.forEach((line: any, lineIndex: number) => {
            template += `
                "{{ msg.address[${index}].line[${lineIndex}] }}"`;
            if (lineIndex < addr.line.length - 1) template += `,`;
          });
          template += `
            ],`;
        }
        if (addr.city) template += `
            "city": "{{ msg.address[${index}].city }}",`;
        if (addr.state) template += `
            "state": "{{ msg.address[${index}].state }}",`;
        if (addr.postalCode) template += `
            "postalCode": "{{ msg.address[${index}].postalCode }}",`;
        if (addr.country) template += `
            "country": "{{ msg.address[${index}].country }}"`;
        template += `
        }`;
        if (index < json.address.length - 1) template += `,`;
      });
      template += `
    ],`;
    }
  ...
  private buildTemplateContext(detectedFields: DetectedField[]): TemplateContext {
  ...
  private createLiquidTemplate(context: TemplateContext): string {
    let template = `{
    "resourceType": "Patient",`;
    ...
    "identifier": [`;
      ...
      context.identifiers.forEach((id, index) => {
        ...
        if (index < context.identifiers.length - 1) template += ',';
      });
      ...
      Object.entries(addressComponents).forEach(([use, components], index) => {
        ...
        if (lineComponents.length > 0) {
          ...
          lineComponents.forEach((comp, i) => {
            ...
            if (i < lineComponents.length - 1) template += ', ';
          });
          template += `],`;
        }
        ...
        if (index < Object.keys(addressComponents).length - 1) template += ',';
      });
    ...
    "extension": [
      ...
      context.clinical.forEach((vital, index) => {
        ...
        if (index < context.clinical.length - 1) template += `,`;
        template += `
                {% endif -%}`;
      });
    ...
    "extension": [`;
      ...
      context.extensions.forEach((ext, index) => {
        ...
        if (index < context.extensions.length - 1) template += `,`;
        template += `
        {% endif -%}`;
      });
  ...
  private getLiquidPath(path: string): string {
    // Convert dot notation to Liquid notation
    return `msg.${path}`;
  }

  private getIdentifierTypeCode(system: string): string {
  ...
  private getIdentifierSystem(system: string): string {
  ...
  private getIdentifierTypeDisplay(system: string): string {
  ...
  private groupAddressComponents(addressFields: Array<{ path: string; component: string; use: string }>): { [use: string]: Array<{ path: string; component: string }> } {
    const grouped: { [use: string]: Array<{ path: string; component: string }> } = {};
  ...
  public analyzeAndGenerate(sampleJson: any): { template: string; analysis: any; confidence: number } {
    // Check if input is already FHIR-structured
    if (this.isFhirStructured(sampleJson)) {
